...
对象表示在页面什么位置显示内容。对象和变量使用双花括号：{% raw %}`{{ objects }}`{% endraw %}。
...
一般来说，即使没有输出文本，Liquid的行也会输出一个空行。可以在标签中加短横，{% raw %}` {{-, -}}, {%-, and -%}` {% endraw %}来去掉输出的空格。例如：
...
在两个注释标签之间的{% raw %}`{% comment %}` {% endraw %}任何文本和代码都不会被解释。例如：
...
Jekyll使用Liquid对文本进行解析，在Markdonw文件中的Liquid代码会被解析，所以，Markdonw文件中的Liquid代码需要使用raw标签进行包裹。raw标签遇到endraw标签就会闭合，如果在代码块中显示<code>{&#37; raw &#37;}</code>和<code>{&#37; endraw &#37;}</code>，可以使用HTML的硬编码（参看本文），如果在代码块中显示，定义一个变量{% raw %}`{% assign openTag = '{%' %}`{% endraw %}，然后使用{% raw %}`{{ openTag }} endraw %}`{% endraw %}
